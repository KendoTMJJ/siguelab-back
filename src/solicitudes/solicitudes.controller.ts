...
export class SolicitudesController {
  ...
  @ApiOperation({
    summary:
      'Crear una reserva directa (admin): queda aprobada de inmediato, sin firmas ni antelación mínima',
  })
  @ApiResponse({ status: 201, description: 'Reserva creada y aprobada' })
  @ApiResponse({
    status: 400,
    description: 'Datos inválidos (reglas de negocio)',
  })
  ...
  crearDirecta(
    @Body() createReservaDirectaDto: CreateReservaDirectaDto,
    @CurrentUser() usuario: AuthenticatedUser,
  ) {
    return this.solicitudesService.crearDirecta(
      createReservaDirectaDto,
      usuario,
    );
  }
